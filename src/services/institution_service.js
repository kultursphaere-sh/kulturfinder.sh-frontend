...
function getHolidayNameById({ id }) {
  ...
  return i18n.te(holidayId)
    ? i18n.t(holidayId)
    : false
}

class InstitutionService {
  constructor(digiCultUrl) {
    this.digiCultUrl = digiCultUrl || 'https://kultursphaere.sh/corsproxy.php?url=http://xtree-actor-api.digicult-verbund.de'
  }

  async fetchInstitutions() {
    const locale = i18n.locale
    const response = await Vue.http.get(`${this.digiCultUrl}/getRepositoryList?portalURI=http://digicult.vocnet.org/portal/p0287&count=9999&lang=${locale}`,
      {
        timeout: TIMEOUT
      })
    ...
    return Promise.all(data.Actor.map(async institution => {
      ...
      if (institution.classification) {
        ...
        const categories = await getCategories(categoryUris)
  ...
  async fetchInstitution(id, locale) {
    const response = id.substr(0, 3) === 'act'
      ? await Vue.http.get(`${this.digiCultUrl}/getRepositoryItem?id=${id}&lang=${locale}`)
      : await Vue.http.get(`${this.digiCultUrl}/getRepositoryItem?info030=${id}&lang=${locale}`)
    ...
    console.log('Details geholt: ', institution)
